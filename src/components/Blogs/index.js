import { getAllBlogs, getAllUnapprovedBlogs } from "@/api/blogs";
...
export default function Blogs() {
  ...
  const getData = async () => {
    try {
      ...
      {
        tab === 1
          ? (response = await getAllBlogs(limit, skip))
          : (response = await getAllUnapprovedBlogs(limit, skip));
      }
  ...
  return (
    <div className="relative  ">
      <div className="max-w-sm w-full grid grid-cols-2 rounded-md my-5">
        <div
          className={
            tab === 1
              ? "p-2 bg-bluePrimary text-white border text-center rounded-l-md"
              : "p-2 bg-white border text-center rounded-l-md"
          }
          onClick={() => setTab(1)}
        >
          Approved
        ...
        <div
          className={
            tab === 2
              ? "p-2 bg-bluePrimary text-white border text-center rounded-r-md"
              : "p-2 bg-white border text-center rounded-r-md"
          }
          onClick={() => setTab(2)}
        >
          Unapproved
                      ...
                      <p className="text-sm font-medium text-gray-900">
                        {post._id ? (
                          <a
                            ...
                            target="_blank"
                            href={`https://admin.test.vitmeds.com/blog/${post._id}`}
                          >
                            {post._id}
                          </a>
                        ) : (
