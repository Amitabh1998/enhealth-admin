...
export default function NewCategoryDialog({
  ...
  const saveHandler = async () => {
    ...
    if (attachmentsUrl.length === 0) {
      ...
      toast.error("Please enter Category", "bottom-right");
    } else {
      ...
      let config = {
        ...
        url: `${process.env.NEXT_PUBLIC_API_URL}master-data/medicine-category${
          datum ? `/${datum?._id}` : ""
        }`,
        headers: {
          ...
          <div className="flex  min-h-full items-center justify-center p-4 text-center">
            ...
            >
              <Dialog.Panel className="w-full flex flex-col    max-w-md transform overflow-hidden rounded-2xl bg-white p-6 text-left align-middle shadow-xl transition-all">
                ...
                >
                  <div>{`${
                    datum ? "Edit Category" : "Add New Categories"
                  }`}</div>
                  <XCircleIcon
                ...
                <div className="mt-2 flex-1">
                  ...
                  <div className="w-full ">
                    ...
                    <div className="flex space-x-3">
                      <button
                        onClick={() => setStatus(1)}
                        className={`p-2 rounded-md flex-1 border ${
                          status === 1 ? "border-bluePrimary border-2" : ""
                        }`}
                      >
                        Branded
                      </button>
                      <button
                        onClick={() => setStatus(2)}
                        className={`p-2 rounded-md flex-1 border  ${
                          status === 2 ? "border-bluePrimary border-2" : ""
                        }`}
                      >
                        Generic
                      </button>
