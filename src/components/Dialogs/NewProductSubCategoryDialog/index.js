...
export default function NewProductSubCategoryDialog({
  isOpen,
  setIsOpen,
  setTableData,
  tableData,
  ...
  const getAllStates = async () => {
    ...
    let config = {
      ...
      url: `${process.env.NEXT_PUBLIC_API_URL}master-data/product-category?$limit=-1&$select=name`,
      headers: {
    ...
    if (state === null) {
      ...
      let config = {
        ...
        url: `${
          ...
          datum ? `/${datum._id}` : ""
        }?$populate[0][path]=productCategory&$populate[0][select][0]=name&$populate[0][select][1]=attachment`,
        headers: {
  ...
  useEffect(() => {
    ...
    if (datum) {
      console.log(datum);
      setName(datum?.name);
      setState(datum?.productCategory);
    }
  }, [datum]);
          ...
          <div className="flex  min-h-full items-center justify-center p-4 text-center">
            ...
            >
              <Dialog.Panel className="w-full flex flex-col  max-w-md transform overflow-hidden rounded-2xl bg-white p-6 text-left align-middle shadow-xl transition-all">
                ...
                >
                  <div>{`${datum ? "Edit" : "Add New"} Sub Category`}</div>
                  <XCircleIcon
                ...
                <div className="mt-2 flex-1">
                  ...
                  <div className="w-full ">
                    <label
                      for="first_name"
                      class="block mb-1 mt-2 text-sm font-normal text-gray-600 "
                    >
                      Sub Category Name
                    </label>
                    <input
