    ...
    try {
      ...
      if (status2 === 1) {
        const response = await getAllData(
          limit,
          _skip,
          `medicines/medicine-details?medicineClass=4&$populate[0][path]=manufacturer&$populate[1][path]=category&$populate[2][path]=subCategories&$select[0]=code&$select[1]=name&$select[2]=manufacturer&$select[3]=category&$select[4]=subCategories&$select[5]=averageRating&$select[6]=updatedAt&$select[7]=medicineClass`
        );
