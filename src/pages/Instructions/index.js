...
const index = () => {
  ...
  const fetchData = async () => {
    setLoading(true);
    const data = await getAllData(limit, skip, "instruction");
    setData(data);
  ...
  const handlePageChange = async (pageNumber) => {
    ...
    setLoading(true);
    const data1 = await getAllData(limit, _skip, "instruction");
    setData(data1);
  ...
  const handleDeleteClick = async (id) => {
    ...
    let config = {
      ...
      url: `${process.env.NEXT_PUBLIC_API_URL}master-data/instruction/${id}`,
      headers: {
