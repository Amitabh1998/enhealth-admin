...
const index = () => {
  ...
  const fetchData = async () => {
    setLoading(true);
    const data = await getAllData(limit, skip, "adverse-effect");
    setData(data);
  ...
  const handlePageChange = async (pageNumber) => {
    ...
    setLoading(true);
    const data1 = await getAllData(limit, _skip, "adverse-effect");
    setData(data1);
  ...
  const handleDeleteClick = async (id) => {
    ...
    let config = {
      ...
      url: `${process.env.NEXT_PUBLIC_API_URL}master-data/adverse-effect/${id}`,
      headers: {
    ...
    <div>
      ...
      {isOpen && (
        ...
        <NewNameLinkDialog
          ...
          setData={setData}
          dialogTitle={
            datum ? "Edit Adverse Effect" : "Add new adverse effects"
          }
          path={"adverse-effect"}
          inputTitle={"Adverse Effects"}
          datum={datum}
        />
      )}
    </div>
