  ...
  const acceptHandler = async () => {
    try {
      ...
      const response = await updateCommonData(
        body,
        `knowledge-center/blog/${router?.query?.id}`
      );
      console.log(response);
      toast.success("Blog Approved succesfully");
          ...
          <div className="relative px-4 sm:px-6 lg:px-8">
            {data && data?.status === 2 ? (
              <div className="flex justify-end items-center space-x-5">
                <button
                  className="p-2 px-5 bg-yellow-400 text-white rounded-md"
                  onClick={() => rejectHandler()}
                >
                  Reject
                </button>
                <button
                  className="p-2 px-5 bg-bluePrimary text-white rounded-md"
