...
const index = () => {
  ...
  const fetchData = async () => {
    setLoading(true);
    const data = await getAllData(limit, skip, "disease");
    setData(data);
  ...
  const handlePageChange = async (pageNumber) => {
    ...
    setLoading(true);
    const data1 = await getAllData(limit, _skip, "disease");
    setData(data1);
  ...
  const handleDeleteClick = async (id) => {
    ...
    let config = {
      ...
      url: `${process.env.NEXT_PUBLIC_API_URL}master-data/disease/${id}`,
      headers: {
    ...
    <div>
      ...
      {isOpen && (
        ...
        <NewNameDialog
          ...
          data={data}
          setData={setData}
          dialogTitle={datum ? "Edit Disease" : "Add new disease"}
          path={"disease"}
          inputTitle={"Disease"}
          datum={datum}
        />
