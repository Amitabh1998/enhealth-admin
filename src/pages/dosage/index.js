...
const index = () => {
  ...
  const fetchData = async () => {
    setLoading(true);
    const data = await getAllData(limit, skip, "dosage");
    setData(data);
  ...
  const handlePageChange = async (pageNumber) => {
    ...
    setLoading(true);
    const data1 = await getAllData(limit, _skip, "dosage");
    setData(data1);
  ...
  const handleDeleteClick = async (id) => {
    ...
    let config = {
      ...
      url: `${process.env.NEXT_PUBLIC_API_URL}master-data/dosage/${id}`,
      headers: {
    ...
    <div>
      ...
      {isOpen && (
        ...
        <NewNameDialog
          ...
          data={data}
          setData={setData}
          dialogTitle={datum ? "Edit Dosage" : "Add new dosage"}
          path={"dosage"}
          inputTitle={"Dosage"}
          datum={datum}
        />
