...
const index = () => {
  ...
  const fetchData = async () => {
    setLoading(true);
    const data = await getAllData(limit, skip, "drug-chemistry");
    setData(data);
  ...
  const handlePageChange = async (pageNumber) => {
    ...
    setLoading(true);
    const data1 = await getAllData(limit, _skip, "drug-chemistry");
    setData(data1);
  ...
  const handleDeleteClick = async (id) => {
    ...
    let config = {
      ...
      url: `${process.env.NEXT_PUBLIC_API_URL}master-data/drug-chemistry/${id}`,
      headers: {
  ...
  const saveHandler = async (e, name) => {
    ...
    if (name.trim() === "") {
      toast.error("Please enter drug chemistry", "bottom-right");
    } else {
      const _tableData = [...tableData];
      try {
        const response = await addNewData(name, "drug-chemistry");
    ...
    <div>
      ...
      {isOpen && (
        ...
        <NewNameDialog
          ...
          data={data}
          setData={setData}
          dialogTitle={"Add new drug chemistry"}
          path={"drug-chemistry"}
          inputTitle={"Drug Chemistry"}
        />
      )}
