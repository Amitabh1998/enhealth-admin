  ...
  const getIngredient = async () => {
    try {
      const data = await getDataWithourLimit(
        `master-data/ingredients/${id}?$populate[1][path]=infoBox.medicineCategory&$populate[1][select][0]=name&$populate[1][select][1]=attachment&$populate[1][select][2]=categoryType`
      );
      setIngredient(data);
      console.log(data);
    } catch (error) {
