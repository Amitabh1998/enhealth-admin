...
import DeliveryRatings from "@/components/DeliveryRatings";
  ...
  const getAllData = async () => {
    try {
      ...
      const data = await getData(
        limit,
        skip,
        `profile/delivery-profile-management/${id}`
      );
      setData(data);
      console.log(data);
      setLoading(false);
  ...
  const acceptHandler = async () => {
    try {
      const response = await addCommonData(
        { id: router?.query?.id },
        "profile/verify-delivery-profile"
      );
      toast.success("Delivery agent is verified successfully");
    } catch (error) {
      toast.error(error ? error : "Something went wrong");
    }
  };
    ...
    <div>
      ...
      {loading ? (
        ...
        <div className="w-full">
          <div className="w-full">
            {tab === 1 ? (
              <DeliveryOverview data={data} />
            ) : tab === 2 ? (
              <DeliveryRatings data={data} />
            ) : tab === 3 ? (
