...
const Profile = () => {
  ...
  useEffect(() => {
    if (router && typeof router?.query?.id?.trim() !== "undefined") {
      getAllData();
    } else {
      return;
    }
  ...
  const acceptHandler = async () => {
    try {
      const response = await addCommonData(
        { id: router?.query?.id },
        "profile/verify-vendor-profile"
      );
      toast.success("Vendor is verified successfully");
    } catch (error) {
      toast.error(error ? error : "Something went wrong");
    }
  };
  ...
  const rejectHandler = async () => {
    try {
      ...
      const response = await updateCommonData(
        { status: -1 },
        `user-management/${router?.query?.id}`
      );
    ...
    <div>
      ...
      {loading ? (
        ...
        <div className="w-full">
          <div className="w-full">
            {tab === 1 ? (
              <VendorOverview data={data} />
            ) : tab === 2 ? (
              <Experience data={data} />
            ) : tab === 3 ? (
              <Wallet data={data} profileId={data?.doctorProfile?._id} />
            ) : tab === 4 ? (
          ...
          {data?.status === 2 && (
            <div className="w-full flex justify-end my-5">
              <div className="flex space-x-2">
                <button
                  className="p-2 text-white bg-red-500 rounded-md hover-bg-indigo-700"
