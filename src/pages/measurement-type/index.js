...
import NewMeasurementTypeDialog from "@/components/Dialogs/NewMeasuremetTypeDialog";
...
const index = () => {
  ...
  const fetchData = async () => {
    setLoading(true);
    const data = await getAllData(limit, skip, "measurement-type");
    setData(data);
  ...
  const handlePageChange = async (pageNumber) => {
    ...
    setLoading(true);
    const data1 = await getAllData(limit, _skip, "measurement-type");
    setData(data1);
  ...
  //   try {
  //     const response = await addNewData(name, "measurement-type");
    ...
    <div>
      ...
      {isOpen && (
        ...
        <NewNameDialog
          ...
          data={data}
          setData={setData}
          dialogTitle={"Add new measurement type"}
          path={"measurement-type"}
          inputTitle={"Measurement Type"}
        />
      )}
