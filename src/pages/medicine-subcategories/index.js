...
import NewSubCategoryDialog from "@/components/Dialogs/NewSubCategoryDialog";
...
const cols = [
  ...
  {
    label: "Image",
    key: "medicineCategory.attachment.link",
    dataType: "avatar",
  },
  {
    label: "Category",
  ...
  const getData = async (pageNumber) => {
    try {
      ...
      const response = await getAllData(
        limit,
        _skip,
        `master-data/medicine-sub-category?$populate[0][path]=medicineCategory&$populate[0][select][0]=name&$populate[0][select][1]=attachment`
      );
        ...
        {loading ? (
          ...
          <div className="w-full">
            ...
            <MasterTable
              ...
              deleteUrl={"master-data/medicine-sub-category/"}
              editHandler={(d) => {
                setDatum(d);
                setIsOpen(true);
              }}
