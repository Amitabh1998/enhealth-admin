import { getAllPincodes } from "@/api/pincodes";
import { getAllStates } from "@/api/states";
import PrimaryButton from "@/components/Buttons/PrimaryButton";
import NewPincodeDialog from "@/components/Dialogs/NewPincodeDialog";
...
const index = () => {
  ...
  const fetchData = async () => {
    setLoading(true);
    const data = await getAllPincodes(limit, skip);
    setData(data);
  ...
  const handlePageChange = async (pageNumber) => {
    ...
    // const response = await getAllPincodes(limit, pageNumber - 1);
    setLoading(true);
    const data1 = await getAllPincodes(limit, _skip);
    setData(data1);
