...
import NewProductSubCategoryDialog from "@/components/Dialogs/NewProductSubCategoryDialog";
  ...
  const getData = async (pageNumber) => {
    try {
      ...
      const response = await getAllData(
        limit,
        _skip,
        `master-data/product-sub-category?$populate[0][path]=productCategory&$populate[0][select][0]=name&$populate[0][select][1]=attachment`
      );
    ...
    <div>
      ...
      <div className="flex-1 w-full flex flex-col justify-center items-center py-10">
        <div className="w-full text-left text-2xl font-semibold text-gray-800">
          Health Product Sub Catergories
        </div>
        {loading ? (
          <SpinnerLoader />
        ) : (
          ...
          <div className="w-full">
            ...
            <MasterTable
              ...
              deleteUrl={"master-data/product-sub-category/"}
              editHandler={(d) => {
                setDatum(d);
                setIsOpen(true);
              }}
