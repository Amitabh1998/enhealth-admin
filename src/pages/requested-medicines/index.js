  ...
  const getData = async (pageNumber) => {
    if (searchText?.trim() === "") {
      try {
        ...
        setLoading(true);
        const response = await getAllData(limit, _skip, `medicine-request?`);
    ...
    debounceTimeoutRef.current = setTimeout(async () => {
      try {
        ...
        const response = await getAllData(
          limit,
          _skip,
          `medicine-request?name[$regex]=${encodeURIComponent(
            text
          )}.*&name[$options]=i`
        );
        console.log(response);
    ...
    <div>
      ...
      <div className="flex-1 w-full flex flex-col justify-center items-center py-10">
        <div className="w-full text-left text-2xl font-semibold text-gray-800">
          Requests for new medicines by users
        </div>
        {loading ? (
          <SpinnerLoader />
        ) : (
