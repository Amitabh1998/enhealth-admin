...
const index = () => {
  ...
  const fetchData = async () => {
    setLoading(true);
    const data = await getAllData(limit, skip, "speciality");
    setData(data);
  ...
  const handlePageChange = async (pageNumber) => {
    ...
    // const response = await getAllData(limit, pageNumber - 1);
    setLoading(true);
    const data1 = await getAllData(limit, _skip, "speciality");
    setData(data1);
    ...
    <div>
      ...
      {isOpen && (
        ...
        <NewNameDialog
          ...
          setData={setData}
          image={true}
          dialogTitle={datum ? "Edit Speciality" : "Add new speciality"}
          path={"speciality"}
