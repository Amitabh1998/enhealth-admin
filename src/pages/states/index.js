  ...
  const getData = async (pageNumber) => {
    if (searchText?.trim() === "") {
      try {
        ...
        setLoading(true);
        const response = await getAllData(limit, _skip, `master-data/state?`);
    ...
    debounceTimeoutRef.current = setTimeout(async () => {
      try {
        ...
        const response = await getAllData(
          limit,
          _skip,
          `master-data/state?name[$regex]=${encodeURIComponent(
            text
          )}.*&name[$options]=i`
        );
        console.log(response);
    ...
    <div>
      ...
      {isOpen && (
        <NewNameDialog
          ...
          data={data}
          setData={setData}
          dialogTitle={"Add new State"}
          path={"state"}
          inputTitle={"State Name"}
        />
      )}
      {isEditOpen && (
        <MasterEditDialog
