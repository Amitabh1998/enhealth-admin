...
const index = () => {
  ...
  const fetchData = async () => {
    setLoading(true);
    const data = await getAllData(limit, skip, "surgery-type");
    setData(data);
  ...
  const handlePageChange = async (pageNumber) => {
    ...
    setLoading(true);
    const data1 = await getAllData(limit, _skip, "surgery-type");
    setData(data1);
  ...
  const handleDeleteClick = async (id) => {
    ...
    let config = {
      ...
      url: `${process.env.NEXT_PUBLIC_API_URL}master-data/surgery-type/${id}`,
      headers: {
    ...
    <div>
      ...
      {isOpen && (
        ...
        <NewNameDialog
          ...
          data={data}
          setData={setData}
          dialogTitle={datum ? "Edit Surgery Type" : "Add new surgery types"}
          path={"surgery-type"}
          inputTitle={"Surgery Type"}
          datum={datum}
        />
      )}
    </div>
